...
const { expect } = chai;
const {
  InstructorAssignment, Person, SchoolClass, SchoolYear,
} = db;

const { ROLE_ROOM_PARENT, ROLE_SECONDARY_INSTRUCTOR } = InstructorAssignment.prototype.roleNames;
...
describe('Test InstructorAssignment', () => {
  describe('InstructorAssignment - CRUD', modelTests(InstructorAssignment, {
    fieldToTest: 'role', object: createRandInstructorAssignment(),
  }));

  describe('isInstructor', () => {
    it('isInstructor', async () => {
      const c1 = await InstructorAssignment.create(createRandInstructorAssignment());
  ...
  describe('findInSameSchoolYear', () => {
    it('findInSameSchoolYear', async () => {
      ...
      await expect(InstructorAssignment.create(
        Object.assign(createRandInstructorAssignment(), {
          schoolYearId: schoolYear.id,
          schoolClassId: schoolClass.id,
          instructorId: p2.id,
        }),
      )).to.eventually.be.rejectedWith(/Validation error: Cannot overlap with existing assignment for.*/g);
    });
  });
  ...
  describe('findInstructors', () => {
    it('findInstructors', async () => {
      ...
      await InstructorAssignment.create(
        Object.assign(createRandInstructorAssignment(ROLE_SECONDARY_INSTRUCTOR), {
          schoolYearId: schoolYear.id,
          schoolClassId: schoolClass.id,
          instructorId: p1.id,
        }),
