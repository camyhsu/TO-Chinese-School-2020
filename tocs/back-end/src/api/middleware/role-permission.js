...
const isActionPermitted = async (req, _res, next) => {
  ...
  if (!userRoleNames.includes(roleNames.ROLE_NAME_SUPER_USER)) {
    ...
    const found = (actionsXRoles.get(str) || []).filter((rs) =>
      userRoleNames.includes(rs)
    );
    if (!found.length) {
...
const hasRole = async (req, next, roleName) => {
  const user = await User.getById(req.userId);
  if (
    (await user.hasRole(roleName)) ||
    (await user.hasRole(roleNames.ROLE_NAME_SUPER_USER))
  ) {
    next();
    return;
  }
...
const isStudentParent = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_STUDENT_PARENT);

const isPrincipal = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_PRINCIPAL);

const isAcademicVicePrincipal = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_ACADEMIC_VICE_PRINCIPAL);

const isInstructor = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_INSTRUCTOR);

const isRegistrationOfficer = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_REGISTRATION_OFFICER);

const isAccountingOfficer = (req, _res, next) =>
  hasRole(req, next, roleNames.ROLE_NAME_ACCOUNTING_OFFICER);
