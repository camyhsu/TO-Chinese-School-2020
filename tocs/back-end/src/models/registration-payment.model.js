...
export default (sequelize, Sequelize, fieldsFactory) => {
  ...
  Object.assign(RegistrationPayment.prototype, {
    getStudentStatusFlags() {
      if (this.studentFeePayments) {
        return this.studentFeePayments.reduce(
          (r, c) =>
            r.concat(
              c.student.studentStatusFlags.filter(
                (x) => x.schoolYearId === this.schoolYearId
              )
            ),
          []
        );
  ...
  Object.assign(RegistrationPayment, {
    createWith: async (obj) =>
      RegistrationPayment.create(obj, {
        include: [{ association: RegistrationPayment.StudentFeePayment }],
      }),
    findPaidPaymentsPaidBy: async (paidById) =>
      RegistrationPayment.findAll({
        where: { paid_by_id: paidById, paid: true },
        order: [["updated_at", "DESC"]],
      }),
    getPvaDueInCents: async (schoolYearId) =>
      RegistrationPayment.sum("pva_due_in_cents", {
        where: { school_year_id: schoolYearId, paid: true },
      }),
    getCccaDueInCents: async (schoolYearId) =>
      RegistrationPayment.sum("ccca_due_in_cents", {
        where: { school_year_id: schoolYearId, paid: true },
      }),
    findPaidPaymentsForSchoolYear: async (schoolYearId) => {
      const tmp = await RegistrationPayment.findAll({
        ...
        include: [
          {
            model: sequelize.models.StudentFeePayment,
    ...
    fillInDue: async (obj, paidStudentFeePaymentsInFamilySize, schoolYear) => {
      obj.pvaDueInCents = RegistrationPayment.calculatePvaDueInCents(
        obj,
        paidStudentFeePaymentsInFamilySize,
        schoolYear
      );
      obj.cccaDueInCents = await RegistrationPayment.calculateCccaDueInCents(
        obj,
        paidStudentFeePaymentsInFamilySize,
        schoolYear
      );
    },
    calculatePvaDueInCents: (
      obj,
      paidStudentFeePaymentsInFamilySize,
      schoolYear
    ) => {
      ...
      if (
        paidStudentFeePaymentsInFamilySize === 1 ||
        obj.studentFeePayments.length < 2
      ) {
        return schoolYear.pvaMembershipDueInCents;
      }
      return schoolYear.pvaMembershipDueInCents * 2;
    },
    calculateCccaDueInCents: async (
      obj,
      paidStudentFeePaymentsInFamilySize,
      schoolYear
    ) => {
      ...
      const familyCccaLifetimeMember =
        await sequelize.models.Family.familyCccaLifetimeMember(obj.paid_by_id);
    ...
    calculateGrandTotal: (obj) => {
      let total = obj.studentFeePayments.reduce(
        (r, c) =>
          r + c.registrationFeeInCents + c.bookChargeInCents + c.tuitionInCents,
        0
      );
