export default (sequelize, _Sequelize, fieldsFactory) => {
  const fields = fieldsFactory({
    withId: true,
    withBooleans: [["cccaLifetimeMember", "ccca_lifetime_member"]],
  });
  const Family = sequelize.define("family", fields);
  const getFamilyIdsByPersonIdSql = (personId) =>
    "SELECT id AS family_id FROM families WHERE" +
  ...
  Object.assign(Family, {
    ...
    async getFamilyMemberIds(personId) {
      ...
      sql +=
        " UNION SELECT id AS family_id, parent_two_id AS person_id FROM families";
      sql +=
        " UNION SELECT family_id, child_id AS person_id FROM families_children";
    ...
    async memberStaffInstructorInfo(personId, schoolYearId) {
      ...
      let sql = "SELECT * FROM (";
      sql +=
        "SELECT 'I' AS type,school_year_id,instructor_id AS p_id FROM instructor_assignments";
      sql +=
        " UNION SELECT 'S',school_year_id,person_id FROM staff_assignments)t";
