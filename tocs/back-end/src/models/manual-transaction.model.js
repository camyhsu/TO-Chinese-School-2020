export default (sequelize, Sequelize, fieldsFactory) => {
  ...
  const ManualTransaction = sequelize.define(
    "manual_transaction",
    {
      ...fields,
      amountInCents: {
    ...
    },
    {
      validate: {
        startEndDateOrder() {
          if (
            this.paymentMethod === this.paymentMethods.PAYMENT_METHOD_CHECK &&
            !this.checkNumber
          ) {
  ...
  Object.assign(ManualTransaction, {
    findTransactionBy: async (transactionById) => {
      ...
      if (txs) {
        txs.forEach((tx) =>
          Object.assign(tx.dataValues, {
            amountWithSign: ManualTransaction.amountWithSign(tx),
          })
        );
      }
      return txs;
    ...
    amountWithSign: (tx) =>
      (tx.amountInCents / 100) *
      (tx.transactionType === tx.transactionTypes.TRANSACTION_TYPE_WITHDRAWAL ||
      tx.transactionType === tx.transactionTypes.TRANSACTION_TYPE_OTHER_REFUND
        ? -1
        : 1),
  });
