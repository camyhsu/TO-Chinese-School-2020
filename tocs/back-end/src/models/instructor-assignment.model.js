...
export default (sequelize, Sequelize, fieldsFactory) => {
  ...
  const InstructorAssignment = sequelize.define('instructor_assignment', {
    ...
    validate: {
      ...
      async noOverlapping() {
        ...
        r.forEach((x) => {
          if (dateBetween(x.startDate, this) || dateBetween(this.startDate, x)) {
            throw new Error(`Cannot overlap with existing assignment for ${x.instructor.name()}`);
          }
        });
      },
    },
  ...
  Object.assign(InstructorAssignment.prototype, {
    ...
    isInstructor() {
      return this.role === InstructorAssignment.prototype.roleNames.ROLE_PRIMARY_INSTRUCTOR
        || this.role === InstructorAssignment.prototype.roleNames.ROLE_SECONDARY_INSTRUCTOR;
    },
