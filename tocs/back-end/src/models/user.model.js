...
export default (sequelize, Sequelize, fieldsFactory) => {
  const fields = fieldsFactory({ withId: true, withStrings: ['password_hash', 'password_salt'] });
  ...
  /* Non-prototype */
  Object.assign(User, {
    createWith: async (obj) => User.create(obj,
    ...
    userNameAlreadyExists: async (username) => !!(await User.findFirstBy({ username })),
  });
