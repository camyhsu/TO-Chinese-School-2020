...
const {
  ...
  StudentClassAssignment, StudentFeePayment, StudentStatusFlag, User,
} = db;

const getRegistrationPayment = async (id, allGatewayTransactions) => {
  const gatewayTransactionOption = { model: GatewayTransaction, as: 'gatewayTransactions' };
  ...
  if (registrationPayment && registrationPayment.studentFeePayments) {
    const promises = registrationPayment.studentFeePayments
      .map(async (studentFeePayment) => {
        ...
        const registrationPreference = await student.getRegistrationPreferenceForSchoolYear(schoolYearId);
...
const setSchoolClassBasedOn = async (registrationPreference, schoolYearId, gender) => {
  const activeGradeClasses = await registrationPreference.grade.getActiveGradeClasses(schoolYearId);
  const filtered = activeGradeClasses.filter((s) => s.schoolClassType === registrationPreference.schoolClassType);
  if (!filtered) {
    return null;
  }
  ...
  const c = await StudentClassAssignment
    .getLowestSchoolClassByGender(schoolYearId, gender, activeGradeClasses.map((s) => s.id));
  console.log(c);
  return c && c.id;
};

const createStudentClassAssignments = async (registrationPayment) => {
  const promises = registrationPayment.studentFeePayments.map(async (studentFeePayment) => {
    ...
    let studentClassAssignment = await student.getStudentClassAssignmentForSchoolYear(schoolYearId)
      || { schoolYearId, studentId };
    const registrationPreference = await student.getRegistrationPreferenceForSchoolYear(schoolYearId);
    studentClassAssignment.gradeId = registrationPreference.grade.id;
    studentClassAssignment.schoolClassId = await setSchoolClassBasedOn(registrationPreference,
      schoolYearId, student.gender);
    studentClassAssignment.electiveClassId = registrationPreference.electiveClassId;
    if (!studentClassAssignment.id) {
      studentClassAssignment = await StudentClassAssignment.create(studentClassAssignment);
    } else {
...
export default {
  ...
  initializeRegistrationPayment: async (personId, ids) => {
    if (ids) {
      ...
      const schoolYearId = registrationPreferences && registrationPreferences.length
        && registrationPreferences[0].schoolYear.id;
      const children = await person.findChildren();
      const paidStudentFeePaymentsInFamilyPromises = await Promise.all(children
        .map(async (student) => student.findPaidStudentFeePaymentAsStudentFor(schoolYearId)));
      const paidStudentFeePaymentsInFamily = paidStudentFeePaymentsInFamilyPromises.reduce((r, c) => r.concat(c), []);
      const registrationPayment = {};
      registrationPayment.studentFeePayments = [];
      registrationPayment.paid_by_id = personId;
      let schoolYear = null;

      await registrationPreferences.reduce((p, registrationPreference) => p.then(async () => {
        registrationPayment.schoolYearId = registrationPreference.schoolYear.id;
        const studentFeePayment = {};
        studentFeePayment.studentId = registrationPreference.student.id;
        await StudentFeePayment.fillInTuitionAndFee(studentFeePayment, personId,
          registrationPreference.schoolYear, registrationPreference.grade,
          paidStudentFeePaymentsInFamily.concat(registrationPayment.studentFeePayments));
        registrationPayment.studentFeePayments.push(studentFeePayment);
        schoolYear = registrationPreference.schoolYear;
      }), Promise.resolve());

      await RegistrationPayment.fillInDue(registrationPayment, paidStudentFeePaymentsInFamily.length, schoolYear);
      RegistrationPayment.calculateGrandTotal(registrationPayment);
      const savedRegistrationPayment = await RegistrationPayment.createWith(registrationPayment);
    ...
    const promises = children.map(async (student) => {
      ...
      if (studentStatusFlag && studentStatusFlag.registered) {
        Object.assign(student.dataValues,
          { registrationPreference: await student.getRegistrationPreferenceForSchoolYear(schoolYearId) });
  ...
  savePayment: async (personId, paymentId, obj) => {
    const registrationPayment = await RegistrationPayment.findOne({
      where: { id: paymentId },
      include: [{ model: PaidBy, as: 'paidBy' }, {
        model: StudentFeePayment,
        as: 'studentFeePayments',
        include: [{
          model: Student,
    ...
    if (registrationPayment.getStudentStatusFlags().filter((x) => x.registered).length > 0) {
      throw new Error('At least one student in the attempted payment has already registered.');
    }
    const cv = new CreditCard(obj.number, toExp4Digits(obj.year, obj.month), obj.cardCode).validate();
    ...
    gatewayTransaction.setApprovalStatusBasedOnAuthorizeNetResponse(resp.responseCode);
    ...
    if (gatewayTransaction.approvalStatus === GatewayTransaction.prototype.status.APPROVAL_STATUS_APPROVED) {
