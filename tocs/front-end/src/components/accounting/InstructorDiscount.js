...
const InstructorDiscount = ({ location } = {}) => {
    ...
    const header = [
        ...
        },
        {
            ...
            cell: (row) => {
                if (row.instructor && row.instructor.children && row.instructor.children.length) {
        ...
        },
        {
            ...
            cell: (row) => {
                if (row.instructor && row.instructor.children && row.instructor.children.length > 1) {
        ...
        {
            title: 'Discount',
            cell: (row) => row.instructor.discount || row.instructor.discount === 0 ? dollar(row.instructor.discount) : ''
        },
    ];
