...
const People = () => {
    ...
    useEffect(() => {
        ...
        RegistrationService.getPeople(currentPage, rowsPerPage, filter, sortedProp).then(
            (response) => {
    ...
    return (
        <Card size="xlarge">
            <CardBody>
                <Table header={header} items={content.items} isLoaded={content.isLoaded} error={content.error} sortKey="chineseName" async={{
                    currentPage,
                    filterText: filter,
