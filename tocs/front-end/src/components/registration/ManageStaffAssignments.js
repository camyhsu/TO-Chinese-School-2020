...
const ManageStaffAssignments = () => {
    ...
    return (
        <Card size="medium">
                <CardBody>
                    {content && content.items && content.items.map((schoolYear, index) => {
                        return (
                            <React.Fragment key={'schoolYear-' + index}>
                                <div className="row">
                                    <div className="col-md-12 mb-3 text-center">
                                        <Link to={'/admin/manage-staff-assignment?id=' + schoolYear.id} className='btn btn-light'>Show Staff Assignment for {schoolYear.name}</Link>
                                    </div>
                                </div>
                            </React.Fragment>
                        );
