...
const SchoolClassCount = ({ location } = {}) => {
    ...
    const printPerson = (p) => (<>{formatPersonNames(p)}<br/>{p.email}<br/>{p.homePhone}</>);
    const header = [
        ...
        { title: '教室', prop: 'location' },
        {
            ...
            cell: (row) => {
                if (row.instructorAssignments) {
                    ...
                    return ps && ps.length && ps.map((c, i) => (<React.Fragment key={'pi-' + i}>{(i && (<br/>)) || ''}{printPerson(c.instructor)}</React.Fragment>));
                }
                return null;
            } 
        },
        {
            ...
            cell: (row) => {
                if (row.instructorAssignments) {
                    ...
                    return ps && ps.length && ps.map((c, i) => (<React.Fragment key={'rp-' + i}>{(i && (<br/>)) || ''}{printPerson(c.instructor)}</React.Fragment>));
                }
                return null;
            } 
        },
    ...
    useEffect(() => {
        ...
        RegistrationService.getchoolClassCount(schoolYearId, elective).then(
            (response) => {
                ...
                setTitle(`千橡中文學校  ${response.data.title}學年度  ${elective ? 'Elective Class' : '班級'}人數清單`);
