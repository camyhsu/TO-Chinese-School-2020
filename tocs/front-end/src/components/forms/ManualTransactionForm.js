...
import { required, OptionalField, formatPersonNames } from '../../utils/utilities';
...
const ManualTransactionForm = ({ location: urlLocation } = {}) => {
    ...
    useEffect(() => {
        ...
        if (personId) {
            AccountingService.initializeManualTransaction(personId).then((response) => {
    ...
    const handleSave = (e) => {
        ...
        if (checkBtn.current.context._errors.length === 0) {
            const obj = {
                ...
                transactionById, paymentMethod, checkNumber,
                amount, note,
            };
            dispatch(addManualTransaction(personId, obj)).then(() => {
                setSuccessful(true);
            }).catch(() => {
                setSuccessful(false);
            });
            ...
            <Card size="large">
                <CardBody>
                    <Form onSubmit={handleSave} ref={form}>
                        {!successful && (
                            <div>
                                <CardTitle>{`New Manual Transaction for ${formatPersonNames(content.person)} as Student`}</CardTitle>
                                ...
                                <div className="row">
                                    <div className="form-group col-md-6 mb-3">
                                        ...
                                        <Select
                                            ...
                                            <option value=''></option>
                                            <option value='Textbook Purchase'>Textbook Purchase</option>
                                ...
                                <div className="row">
                                    <div className="form-group col-md-6 mb-3">
                                        ...
                                        <Select
                                            ...
                                            validations={[required]}>
                                            <option value=''></option>
                                            {content.parents && content.parents.map(p => (<option key={`p-${p.id}`} value={p.id}>{formatPersonNames(p)}</option>))}
                                        </Select>
                                    </div>
                                    <div className="form-group col-md-6 mb-3">
                                ...
                                <div className="row">
                                    <div className="form-group col-md-6 mb-3">
                                        <label htmlFor="checkNumber">Check Number <OptionalField /></label>
                                        <Input
                                            type="text"
                                            className="form-control"
                                            name="checkNumber"
