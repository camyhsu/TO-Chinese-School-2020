...
const BookChargeForm = ({ location } = {}) => {
            ...
            dispatch(getBookCharges(schoolYearId)).then((response) => {
                if (response && response.data) {
                    ...
                    if (_bookCharges && _bookCharges.length) {
                        const obj = _bookCharges.reduce((result, current) => Object.assign(result, { [current.id]: current }), {});
                        setBookCharges(obj);
    ...
    const handleSave = (e) => {
        ...
        if (checkBtn.current.context._errors.length === 0) {
            const obj = Object.entries(bookCharges).reduce((result, [key, value]) => Object.assign(result, { [key]: value.bookCharge }), {});
            dispatch(saveBookCharges(schoolYearId, obj)).then(() => {
                setSuccessful(true);
            }).catch(() => {
                setSuccessful(false);
            });
                    ...
                    <Form onSubmit={handleSave} ref={form}>
                        {!successful && (
                            <div>
                                ...
                                {bookCharges && Object.entries(bookCharges).map(([key, value]) => {
                                    return (
