...
const CheckoutHistoryForm = ({ location } = {}) => {
    ...
    useEffect(() => {
        ...
        LibrarianService.getLibraryBookCheckOutHistory(id).then(
            (response) => {
                ...
                if (book && book.eligibleCheckoutPeople && book.eligibleCheckoutPeople.length) {
    ...
    const handleSave = (e) => {
        ...
        if (checkBtn.current.context._errors.length === 0) {
            ...
            dispatch(book.checkedOut ? returnLibraryBook(id, obj) : checkOutLibraryBook(id, obj)).then(() => {
                setSuccessful(true);
            }).catch(() => {
                setSuccessful(false);
            });
                    ...
                    <Card size="large">
                        <CardBody>
                            ...
                            <dl className="row">
                                ...
                                <dt className="col-12 col-md-4 text-left text-md-right">Title:</dt>
                                <dd className="col-12 col-md-8 text-left border-bottom border-md-bottom-0">{book.title}</dd>
                                <dt className="col-12 col-md-4 text-left text-md-right">Description:</dt>
                                <dd className="col-12 col-md-8 text-left border-bottom border-md-bottom-0">{book.description}</dd>
                                <dt className="col-12 col-md-4 text-left text-md-right">Book Type:</dt>
                                <dd className="col-12 col-md-8 text-left border-bottom border-md-bottom-0">{book.bookType}</dd>
                                <dt className="col-12 col-md-4 text-left text-md-right">Checked Out ?</dt>
                                <dd className="col-12 col-md-8 text-left border-bottom border-md-bottom-0">{yesOrNo(book.checkedOut)}</dd>
                                {book.checkedOut && (
                                    <>
                                        <dt className="col-12 col-md-4 text-left text-md-right">Checked Out By:</dt>
                                        <dd className="col-12 col-md-8 text-left border-bottom border-md-bottom-0">{formatPersonNames(currentCheckout.checkedOutBy)}</dd>
                                    </>
                                )}
                            </dl>
                            <div className="card-footer row">
                                <div className="col-md-6">
                                    <Link to="/librarian/books" className="btn btn-light">Back to Library Books</Link>
                                </div>
                            </div>
                        </CardBody>
                    </Card>
                            ...
                            <Form onSubmit={handleSave} ref={form}>
                                {!successful && (
                                    <div>
                                        ...
                                        <div className="row">
                                            <div className="form-group col-md-12 mb-3">
                                                <button className="btn btn-primary btn-block">{book.checkedOut ? 'Return' : 'Check Out'}</button>
                                            </div>
                                        </div>
                                    </div>
                                )}
