...
const SchoolClassStudents = ({ location } = {}) => {
    ...
    const electiveHeader = [
        ...
        { title: '電話', cell: (row) => formatPhoneNumber(row.student.family.address.homePhone) },
    ];
    ...
    useEffect(() => {
        const { schoolYearId: _schoolYearId, id: _id } = queryString.parse(location.search);
        setId(_id);
        ...
        if (id && schoolYearId) {
            InstructionService.getSchoolClass(id, schoolYearId).then(
                (response) => {
                    const { instructorAssignments, schoolClass, studentClassAssignments } = response.data;
                    let primaryInstructor, roomParent;
                    if (instructorAssignments) {
                        primaryInstructor = extract(instructorAssignments, 'Primary Instructor');
                        roomParent = extract(instructorAssignments, 'Room Parent');
                    }

                    setContent({
                        isLoaded: true, schoolClass, studentClassAssignments, primaryInstructor, roomParent
                    });
                },
                (error) => {
                    const _content =
    ...
    return (content.schoolClass && (
        <Card size="no-max-width">
            <CardTitle>{content.schoolClass.chineseName}</CardTitle>
            <CardBody>
                <div className="row">
                    <div className="col-md-4">{`${formatPersonNames(content.primaryInstructor)} 老師`}</div>
                    <div className="col-md-4">教室: {content.schoolClass.location}</div>
                    <div className="col-md-4">Room Parent: {`${formatPersonNames(content.roomParent)}`}</div>
                </div>
                <Table header={content.schoolClass.schoolClassType === 'ELECTIVE' ? electiveHeader : nonElectiveHeader} items={content.studentClassAssignments} isLoaded={content.isLoaded} error={content.error} sortKey="id" showAll="true" />
                
            <dl className="row">
                <dt className="col-12 col-md-6 text-md-right">Total Student Count:</dt>
                <dd className="col-12 col-md-6">{content.studentClassAssignments.length}</dd>
            </dl>
            <dl className="row">
