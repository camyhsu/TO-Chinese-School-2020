  ...
  const header = [
    {
      title: "Grade",
      cell: (row) =>
        bilingualName(row.student.studentClassAssignments[0].grade),
    },
    {
      title: "Class",
      cell: (row) =>
        bilingualName(row.student.studentClassAssignments[0].schoolClass),
    },
  ...
  useEffect(() => {
    ...
    LibrarianService.initializeSearchStudents().then(
      (response) => {
        ...
        setTitle(
          `Search Students By Registration Date For ${schoolYear.name} School Year`
        );
      ...
      <CardBody>
        <Form ref={form}>
          <div className="row">
            ...
            <div className="form-group col-md-2 mb-3">
              <button className="btn btn-primary btn-light" onClick={search}>
                Search
              </button>
            </div>
          </div>
